...
writecell(output_x_initial,'output2/output.txt','WriteMode','append')
for i = 1:ntime
    mod1 = mod(i,4);
    row1 = fix(i/4);
    if mod1 == 0
        ...
        output_x{row1,mod1*2} = char(num2str(loading(i),'%.6f'));
    else
        output_x{row1+1,mod1*2-1} = char(num2str(t(i),15));
        output_x{row1+1,mod1*2} = char(num2str(loading(i),'%.6f'));
    end
end
output_x_middel = output_x(1:end-1,:);
writecell(output_x_middel,'output2/output.txt','WriteMode','append')
for i=1:mod1
    output_x_end{1,i*2-1} = output_x{row1+1,i*2-1};
    output_x_end{1,i*2} = output_x{row1+1,i*2};
end
writecell(output_x_end,'output2/output.txt','WriteMode','append')
ret = 'finish';
